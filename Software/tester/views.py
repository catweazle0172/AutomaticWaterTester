    ...
    if request.method=='POST':
        ...
        if testAction=='EDIT':
            ...
            if testDef.is_valid():                
                ...
                if (testDefToSave.reagent1Amount-testDefToSave.reagent1Amount>0):
                    messages.error(request,'Reagent1 must be an integer number of drops')
                    retryTestSetup=True
                elif (testDefToSave.reagent2Amount-testDefToSave.reagent2Amount>0):
                    messages.error(request,'Reagent1 must be an integer number of drops')
                    retryTestSetup=True
                elif (testDefToSave.reagent3Amount-testDefToSave.reagent3Amount>0):
                    messages.error(request,'Reagent1 must be an integer number of drops')
                    retryTestSetup=True
                elif (testDefToSave.titrationMaxAmount-testDefToSave.titrationMaxAmount>0):
    ...
    if request.method=='POST':
        ...
        if adminAction=='CANCEL':
            ...
            try:
                ...
                sendCmdToTester('ALARMS/testTelegram/' + newKey)
